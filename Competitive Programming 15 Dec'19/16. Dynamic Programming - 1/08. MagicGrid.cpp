...
int magicGrid(int **grid, int r, int c, int si, int sj, int **dp){
    if(si == r-1 && sj == c-1){
        dp[si][sj] = max(1, 1-grid[r-1][c-1]);
        return dp[si][sj];
    }
    if(si>r-1 || sj>c-1){
        return INT_MAX;
    ...
    int option1 = magicGrid(grid, r, c, si, sj+1, dp);
    int option2 = magicGrid(grid, r, c, si+1, sj, dp);
    dp[si][sj] = max(1, min(option1, option2)-grid[si][sj]);
    return dp[si][sj];
}
int main(){
    int t;
    ...
    while(t--){
        ...
        cout<< magicGrid(grid, r, c, 0, 0, dp)<< endl;
    }
    return 0;
}
